...
exports.createMCQ = asyncHandler(async(req,res)=>{
    ...
    try{
        const mcq=new  MCQ({question,subject,difficulty,options,correctOption});
        await mcq.save();
...
exports.updateMCQ = asyncHandler(async(req,res)=>{
    ...
    const {question,subject,difficulty,options,correctOption}=req.body;
    try{
        const mcq=await MCQ.findByIdAndUpdate(id,{question,subject,difficulty,options,correctOption},{new:true});
        if(!mcq) return res.status(404).send('MCQ not found');
